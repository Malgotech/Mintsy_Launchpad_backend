...
const LIVEKIT_API_KEY = "APIuaFxyScyBENq";
const LIVEKIT_API_SECRET = "LvBp4UlP266d9GdGreThn0fyKcPwRa85RdM0In9H0rw";
router.post("/token", async (req, res) => {
  ...
  if (!LIVEKIT_API_KEY || !LIVEKIT_API_SECRET) {
    return res
      .status(500)
      .json({ error: "LiveKit credentials not set in environment" });
  }
  try {
    const at = new livekit_server_sdk_1.AccessToken(
      LIVEKIT_API_KEY,
      LIVEKIT_API_SECRET,
      {
        identity: user,
