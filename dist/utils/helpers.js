...
async function createBaselineCandle(token, startTime, interval) {
    // Validate inputs
    if (!token || !startTime) {
        throw new Error("Invalid token or startTime provided to createBaselineCandle");
    }
    ...
    const is_5_min = interval === "5m" || (interval === "1m" && (timestamp / 60) % 5 === 0);
    return {
...
function calculateMarketCapFromTrade(trade, solUsdPrice) {
...
async function generateOHLCVCandles(trades, interval, startTime) {
    ...
    if (!Array.isArray(trades)) {
        console.warn("generateOHLCVCandles: trades is not an array, returning empty array");
        return [];
    }
    const livePriceDB = await prismaService_1.prisma.liveSolPrice.findUnique({
        where: { symbol: "SOL" },
    ...
    trades.forEach((trade) => {
        ...
        const now = Date.now();
        if (tradeTime > now) {
            console.warn("Trade timestamp is in the future (fake timestamp), skipping:", trade.id, "timestamp:", new Date(tradeTime));
            return;
        }
