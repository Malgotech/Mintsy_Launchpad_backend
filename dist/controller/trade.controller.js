...
class TradeController {
    constructor() {
        ...
        //   }
        // };
        this.placeOrder = async (req, res) => {
            try {
                const { userId, tokenId, solAmount, tokenAmount, side, txHash, marketCap, } = req.body;
                if (!userId || !tokenId || !side || !txHash) {
                    return res
                        .status(400)
                        .json({ success: false, error: "Missing required fields" });
                ...
                let solAmt = typeof solAmount === "string" ? Number(solAmount) : solAmount;
                let tokenAmt = typeof tokenAmount === "string" ? Number(tokenAmount) : tokenAmount;
                let marketCapF = Number(marketCap);
                console.log("solAmt,tokenAmount", marketCapF, solAmt, tokenAmount, marketCapF);
                ...
                const solVolume = trades.reduce((sum, t) => sum + Number(t.price || 0), 0);
                const volume = solVolume * solPrice;
                (0, socket_1.getIO)().emit("trade-created", {
                ...
                (0, socket_1.emitChartUpdate)(tokenIdInt.toString());
                (0, socket_2.emitChartUpdateForCoin)(tokenIdInt.toString());
        ...
        this.getTradeHistory = async (req, res) => {
            try {
        ...
        this.getDailyVolume = async (req, res) => {
            try {
