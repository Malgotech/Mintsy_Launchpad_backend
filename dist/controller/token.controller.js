  ...
  constructor() {
    ...
    this.getTokenById = async (req, res) => {
      try {
        ...
        const coinDetails = {
          ...
          finalMarketCap: token.finalMarketCap * solPrice,
          creator: {
            id: token.creator.id,
      ...
      try {
        ...
        if (lastTrade) {
          whereClause.lastTradeAt = {
            gte: new Date(
              Date.now() - parseInt(lastTrade) * 24 * 60 * 60 * 1000
            ),
          };
        }
        if (creationTime) {
          whereClause.createdAt = {
            gte: new Date(
              Date.now() - parseInt(creationTime) * 24 * 60 * 60 * 1000
            ),
          };
        }
        if (sortBy) {
    ...
    this.updateToken = async (req, res) => {
      try {
        ...
        (0, socket_1.getIO)().emit("token-update", {
          token: token,
          market: token.market,
        });
        return res.json({
