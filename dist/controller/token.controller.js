...
class TokenController {
    constructor() {
            ...
            try {
                ...
                if (fullToken) {
                    ...
                    if (tokenSupply > 0) {
                        ...
                        const top10Sum = top10Holders.reduce((sum, ut) => sum + (ut.tokenAmount || 0), 0);
                        top10 = Number(((top10Sum / tokenSupply) * 100).toFixed(2));
                    }
                    (0, socket_1.getIO)().emit("token-created", {
                        channel: "new-token",
            ...
            try {
                ...
                if (tags) {
                    ...
                    whereClause.tags = {
                        some: {
                            id: {
                                in: tagArray.map((tag) => String(tag).toLowerCase().replace(/\s+/g, "-")),
                            },
                        },
                    };
                }
                ...
                const tokensWithVolume = tokens.map((token) => ({
                    ...token,
        ...
        // }
        /// new type
        this.getTokenById = async (req, res) => {
            try {
                const { id } = req.params;
                const token = await prisma.token.findUnique({
                ...
                const coinDetails = {
                    ...
                    market: token.market
                        ...
                        : null,
                    socials: token.socials?.reduce((acc, curr) => {
        ...
        this.getAllTokensByFilter = async (req, res) => {
            try {
                const { featured, lastTrade, currentlyLive, creationTime, sortBy, order, } = req.query;
                let whereClause = {};
                let orderBy = {};
                if (featured === "true") {
                ...
                if (lastTrade) {
                    whereClause.lastTradeAt = {
                        gte: new Date(Date.now() - parseInt(lastTrade) * 24 * 60 * 60 * 1000),
                    };
                }
                if (creationTime) {
                    whereClause.createdAt = {
                        gte: new Date(Date.now() - parseInt(creationTime) * 24 * 60 * 60 * 1000),
                    };
                }
                if (sortBy) {
        ...
        this.updateToken = async (req, res) => {
            try {
                const { id } = req.params;
                const { name, progress, userCount, durationInDays, apy, marketCap, currentPrice, volume24h, liquidityPercentage, priceChange24h, commentCount, status, nsfw, network, creator, isLive, mintAccount, } = req.body;
                ...
                if (isLive === false) {
                    await prismaService_1.default.deleteLiveStreamChatMessagesByToken(id);
                }
                // Emit chart updates when token data changes
                try {
                    await (0, socket_2.emitChartUpdateOnPriceChange)(Number(id));
                }
                catch (error) {
                    console.error("Error emitting chart update on token change:", error);
                }
                // Emit websocket event for token update
                (0, socket_1.getIO)().emit("token-update", { token: token, market: token.market });
                return res.json({
                    success: true,
                    data: token,
                });
        ...
        this.updateTokenProgress = async (req, res) => {
            try {
                const { id } = req.params;
                const { progress } = req.body;
                console.log(typeof progress !== "number" || progress < 0 || progress > 100);
                ...
                (0, socket_1.getIO)().emit("token-progress-update", { token });
                return res.json({
                    success: true,
                    data: token,
                });
        ...
        this.getSimilarCoins = async (req, res) => {
            try {
                ...
                const scoredTokens = tokens.map((token) => {
                    const tokenWords = new Set(token
                        .description.toLowerCase()
                        .split(/\s+/)
                        .filter((word) => word.length > 3));
                    const matchScore = [...searchWordSet].filter((word) => tokenWords.has(word)).length;
                    return {
                        ...token,
                        matchScore,
        ...
        this.getCoinDetails = async (req, res) => {
            try {
        ...
        this.updateLpStatus = async (req, res) => {
            try {
                const { id } = req.params;
        ...
        this.getTokenHolders = async (req, res) => {
            try {
                const { id } = req.params;
                const trades = await prisma.trade.findMany({
                    where: {
                ...
                const holders = trades.reduce((acc, trade) => {
                    if (!acc[trade.userId]) {
                        acc[trade.userId] = {
                            ...trade.user,
                            amount: 0,
                ...
                const holderList = Object.values(holders).filter((holder) => holder.amount > 0);
                return res.json({
                    success: true,
                    data: holderList,
                });
        ...
        this.getTopHoldersByCoinId = async (req, res) => {
            try {
                ...
                const topHolders = token.userTokens.map((userToken) => ({
        ...
        // Get live tokens
        this.getLiveTokens = async (req, res) => {
            try {
                ...
                const volumeMap = Object.fromEntries(volumes.map((v) => [v.tokenId, v.volume]));
                // Attach volume to each token
                const tokensWithVolume = liveTokens.map((token) => ({
                    ...token,
        ...
        // Get tokens by tag
        this.getTokensByTag = async (req, res) => {
            try {
        ...
        this.updateGraduationStatus = async (req, res) => {
            try {
                ...
                try {
                    const response = await axios_1.default.post("https://api.mintsyplus.fun/addGraduatedToken", {
                    ...
                    if (trades.length > 0) {
                        try {
                            const response = await axios_1.default.post("https://api.mintsyplus.fun/addTrade", trades, // send the array directly
                            { headers: { "Content-Type": "application/json" } });
                        }
                        catch (axiosError) {
                            console.error("Error calling addGraduatedTrade API:", axiosError.response?.data || axiosError.message);
                        }
                    }
                    else {
                    ...
                    console.error("Error calling addGraduatedToken API:", axiosError.message);
                }
                return res.json({
                    success: true,
