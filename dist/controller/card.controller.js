...
class CardController {
    constructor() {
        ...
        this.getAlphaSpot = async (req, res) => {
            try {
                const { sort = "graduatingsoon", limit = 10, offset = 0, filter, } = req.query;
                // Normalize sort value
                const sortValue = typeof sort === "string" ? sort.toLowerCase() : "graduatingsoon";
                let whereClause = {
                    status: "ACTIVE",
                    // progress: {
                    //   gt: 30,
                ...
                const userCounts = await Promise.all(tokens.map(async (token) => {
                ...
                const top10Percents = await Promise.all(tokens.map(async (token) => {
                ...
                const threadCounts = await Promise.all(tokens.map(async (token) => {
                ...
                const threadCountMap = Object.fromEntries(threadCounts.map((tc) => [tc.tokenId, tc.threadCount]));
                ...
                const cards = tokens.map((token) => ({
                    ...
                    //@ts-ignore
                    replies: token.socials?.reduce((acc, s) => acc + (s.commentCount || 0), 0) || 0,
                    ...
                    createdAtDisplay: `Created ${(0, helpers_1.getTimeAgo)(token.createdAt)}`,
                    change: token.market?.priceChange24h
                        ? `${token.market.priceChange24h > 0 ? "+" : ""}${token.market.priceChange24h}`
                        : "+0.00",
        ...
        this.getTrendingCards = async (req, res) => {
            try {
                ...
                const cards = tokens.map((token) => ({
                    ...
                    tags: token.tags,
                    createdAt: token.createdAt,
                    timeAgo: (0, helpers_1.getTimeAgo)(token.createdAt),
                    stats: {
                        users: userCountMap[token.id] || 0,
                        // top10: top10Map[token.id] || 0,
                    },
        ...
        this.getFeaturedCards = async (req, res) => {
            try {
                ...
                const cards = tokens.map((token) => ({
                    ...
                    tags: token.tags.map((tag) => tag.id),
                    network: token.network || "solana",
                    progress: token.progress,
                    isLive: token.isLive || false,
                    lastTrade: token.trades?.[0]
                        ? (0, helpers_1.getTimeAgo)(token.trades[0].createdAt)
                        : null,
                ...
                const formattedTags = tagCounts.map((tag) => ({
                    id: tag.id,
                    label: tag.label,
                    active: tags ? tags.split(",").includes(tag.id) : false,
        ...
        this.getFilteredTokens = async (req, res) => {
            try {
                const { filter, tags, nsfw = false } = req.query;
                let whereClause = {};
                if (tags) {
                ...
                if (filter === "LastTrade") {
                    tokens.sort((a, b) => {
                        const aLastTrade = a.trades?.sort((x, y) => new Date(y.createdAt).getTime() - new Date(x.createdAt).getTime())[0]?.createdAt;
                        const bLastTrade = b.trades?.sort((x, y) => new Date(y.createdAt).getTime() - new Date(x.createdAt).getTime())[0]?.createdAt;
                        if (!aLastTrade && !bLastTrade)
                            return 0;
                        if (!aLastTrade)
                            return 1;
                ...
                if (filter === "LastReply") {
                    ...
                    const tokenLastReplies = await Promise.all(tokens.map(async (token) => {
                        ...
                        if (lastThreadTime && lastReplyTime) {
                            return {
                                tokenId: token.id,
                                lastActivityTime: lastReplyTime > lastThreadTime
                                    ? lastReplyTime
                                    : lastThreadTime,
                            };
                        }
                    ...
                    tokens.sort((a, b) => {
                        const aActivity = tokenLastReplies.find((t) => t.tokenId === a.id)?.lastActivityTime;
                        const bActivity = tokenLastReplies.find((t) => t.tokenId === b.id)?.lastActivityTime;
        ...
        this.getHiddenCards = async (req, res) => {
            try {
