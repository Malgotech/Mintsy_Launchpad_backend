...
export class WatchlistController {
  ...
  addToWatchlist = async (req: Request, res: Response) => {
    try {
      ...
      const alreadyExists = watchlist.tokens.some(
        (token: { id: number }) => token.id === tokenId,
      );
      if (alreadyExists) {
        return res
          .status(400)
  ...
  getUserLists = async (req: Request, res: Response) => {
    try {
      ...
      const hiddenTokenIds = hidden.map((h: { tokenId: any }) => h.tokenId);

      // Gather all visible token IDs across all lists
      const allVisibleTokenIds = Array.from(
        new Set(
          watchlists
            .flatMap((list: { tokens: any[] }) =>
              list.tokens.map((token: { id: any }) => token.id),
            )
            .filter((id: any) => !hiddenTokenIds.includes(id)),
        ),
      );
      ...
      const supplyMap = Object.fromEntries(
        tokenSupplies.map((t: { id: any; supply: any }) => [t.id, t.supply]),
      );
      const top10Percents = await Promise.all(
        allVisibleTokenIds.map(async (tokenId) => {
          const supply = supplyMap[tokenId as number] || 0;
      ...
      const formattedLists = watchlists.map((list: any) => ({
        id: list.id,
        name: list.name,
        type: "default",
        // Filter out hidden tokens and add stats
        tokens: list.tokens
          .filter((token: { id: any }) => !hiddenTokenIds.includes(token.id))
          .map((token: { id: string | number }) => ({
            ...token,
            stats: {
        ...
        cardCount: list.tokens.filter(
          (token: { id: any }) => !hiddenTokenIds.includes(token.id),
        ).length,
